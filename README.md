...
just today's orders automatically.
...
- `api/order.js`, runs on Vercel's servers when someone submits the order
  form. It's the only piece that talks to Google Sheets and Google Drive.
- `package.json`, the one dependency (`googleapis`) the order function needs.
...
## One-time setup: connecting the site to Google Sheets and Drive
...
```
Date Submitted | Customer Name | Contact Number | Order Date Needed | Pickup or Delivery | Order Details | Notes | Receipt | Paid
```

Add a second tab called **Today's Orders**. In cell A1 of that tab, paste:

```
=FILTER('All Orders'!A2:I, INT('All Orders'!A2:A) = INT(TODAY()))
```
...
### 2. Make a Google Drive folder for payment receipts

Create a folder in Google Drive (call it something like "Payment
Receipts"). Every screenshot customers upload will land here. Grab its
**Folder ID** the same way as the Sheet ID, it's the long string in the
folder's URL after `/folders/`.

### 3. Create a "service account" (a robot user that can write to the sheet and folder)
...
### 4. Share the sheet and the Drive folder with the service account
...
Do the exact same thing for the Drive folder from step 2, share it with
that same service account email, **Editor** access.

### 5. Add the secrets to Vercel
...
- **Receipt links are "anyone with the link can view."** That keeps setup
  simple (no need to individually share every file), but it does mean the
  link isn't locked down tighter than that. Fine for a small business's
  internal use, worth knowing if that ever matters.
