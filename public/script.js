...
// --- Lightbox: click a gallery photo to see it full size ---
...
function addToCart(name, price) {
  ...
  if (existing) {
    ...
    cart.push({ name, price: price === null || isNaN(price) ? null : price, qty: 1 });
  }
  saveCart();
  renderCart();
}
...
// Add-to-cart buttons: bundles & sets (no fixed price known, shown on the photo)
document.querySelectorAll('.add-cart-btn-gallery').forEach((btn) => {
  btn.addEventListener('click', () => {
    addToCart(btn.dataset.name, null);
...
cartCheckoutBtn.addEventListener('click', () => {
  ...
  const lines = cart.map((it) => `${it.qty}x ${it.name}${it.price !== null ? ' (\u20B1' + (it.price * it.qty) + ')' : ''}`);
  const total = pricedTotal();
  let summary = lines.join('\n');
  if (total > 0) {
    summary += `\n\nEstimated total (\u00e0 la carte items only): \u20B1${total}`;
...
function openPayment() {
  ...
  if (total > 0) {
    ...
    paymentAmountBox.innerHTML = `Please pay <span class="amount">\u20B1${downpayment}</span><span class="note">50% downpayment of your \u20B1${total} order (\u00e0 la carte items). Bundle/set amounts will be confirmed separately.</span>`;
  } else {
    paymentAmountBox.innerHTML = `<span class="note">We'll confirm your downpayment amount with you directly, since your order includes bundle/set items with prices shown on their photos.</span>`;
  }
